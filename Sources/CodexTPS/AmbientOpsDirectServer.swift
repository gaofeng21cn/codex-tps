...
final class AmbientOpsDirectServer: @unchecked Sendable {
  ...
  private let queue = DispatchQueue(label: "io.github.gaofeng21cn.codex-tps.direct")
...
private final class AmbientOpsDirectHTTPConnection: @unchecked Sendable {
  ...
  private func handle(_ request: Data) {
    ...
    Task {
      ...
      switch path {
      case "/healthz":
        let body = Data(#"{"ok":true,"provider":"codex-tps","scope":"machine"}"#.utf8)
        send(
          status: 200,
          reason: "OK",
          body: body,
